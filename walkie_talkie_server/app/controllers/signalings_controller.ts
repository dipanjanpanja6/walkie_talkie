...
export default class SignalingsController {
  ...
  public async onDisconnected() {
    ...
    try {
      ...
      if (this.socket.data.userId && this.socket.data.channelId) {
        await this.handleUserLeave(this.socket)
      }
    } catch (error) {
  ...
  public async joinChannel(data: any) {
    try {
      ...
      const userSession = await UserSession.updateOrCreate(
        { userId, channelId },
        {
      ...
      this.socket.emit('joined-channel', {
        channelId,
        userId,
        routerCapabilities,
      ...
      this.socket.to(`channel:${channelId}`).emit('user-joined', {
        userId,
        displayName,
        joinedAt: new Date(),
      })
  ...
  public async produce(data: any) {
    try {
      ...
      this.socket.to(`channel:${this.socket.data.channelId}`).emit('new-producer', {
  ...
  public async consume(data: any) {
    try {
      ...
      const consumerParams = await this.mediasoupService.createConsumer(
        this.socket.data.channelId,
        this.socket.data.userId,
        transportId,
        producerId,
  ...
  public async handleIceCandidate(data: any) {
    try {
      ...
      this.socket.to(`channel:${this.socket.data.channelId}`).emit('ice-candidate', {
        candidate,
        fromUserId: this.socket.data.userId,
      })
  ...
  private async handleUserLeave() {
    try {
      ...
      await UserSession.query()
        ...
        .update({ isActive: false, leftAt: new Date() })
